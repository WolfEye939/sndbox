...
const mockUpStrand = () => {
  const newStrand = []
  for (let i = 0; i < 15; i++) { newStrand.push(returnRandBase()) }
  return newStrand
};
  ...
  return {
    ...
    mutate() {
      ...
      if (this._dna[randomBase] === 'A') 
        ...
        else if (this.dna[randomBase] === 'C') { this._dna[randomBase] = dnaBasesNoC[Math.floor(Math.random() * 3)]; }
      else { this._dna[randomBase] = dnaBasesNoG[Math.floor(Math.random() * 3)]; }
      return this._dna;
    },
    ...
    willLikelySurvive() {
      ...
      { if (this._dna[i] === 'C' || this._dna[i] === 'G') { cAndGCounter++; } }
      if (cAndGCounter < 9) 
        {return false; }
      else { return true; }
    }
...
while (survivors.length < 30) {
  ...
  if (newPAequor.willLikelySurvive() === true) { survivors.push(newPAequor); }
}
